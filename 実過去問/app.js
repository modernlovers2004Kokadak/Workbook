...
let pastNavigationReady=false,pastInternalDepth=0,pendingPastHome=false;
...
function commitPastNavigation(replace=false){try{history[replace?'replaceState':'pushState']({pastApp:true,depth:pastInternalDepth,state:pastNavigationState()},'',location.href)}catch(_){}}
function restorePastNavigation(saved){if(!saved)return;state.screen=saved.screen||'home';state.mode=saved.mode==='exam'?'exam':'study';state.exam=EXAMS.find(e=>examKey(e)===String(saved.examKey))||EXAM53;state.index=Math.min(Math.max(0,Number(saved.index)||0),state.exam.count-1);state.answers=Array.isArray(saved.answers)?saved.answers:Array(state.exam.count).fill(null);state.revealed=!!saved.revealed;render()}
function pastHome(){if(pastInternalDepth>0){pendingPastHome=true;history.go(-pastInternalDepth);return}location.replace('../index.html')}
...
function start(key,mode){const existing=pendingPast();if(pastNavigationReady)commitPastNavigation(true);if(existing&&String(existing.examKey)===String(key)&&existing.mode===mode){restoreSession();render();if(pastNavigationReady){pastInternalDepth++;commitPastNavigation()}return}if(existing&&!confirm('未完了の過去問があります。最初から開始しますか？'))return;state.screen='quiz';state.exam=EXAMS.find(e=>examKey(e)===String(key))||EXAM53;state.mode=mode;state.index=0;state.answers=Array(state.exam.count).fill(null);state.revealed=false;saveSession();render();if(pastNavigationReady){pastInternalDepth++;commitPastNavigation()}}
...
function next(){if(state.answers[state.index]===null)return;if(state.index===state.exam.count-1){state.screen='result';clearSession();render();commitPastNavigation(true);return}state.index++;state.revealed=false;saveSession();render();commitPastNavigation(true);scrollTo(0,0)}
function previousQuestion(){if(state.index<=0)return;state.index--;state.revealed=state.mode==='study'&&state.answers[state.index]!==null;saveSession();render();commitPastNavigation(true);scrollTo(0,0)}
...
function showHome(){state.screen='home';render();scrollTo(0,0);if(pastNavigationReady){pastInternalDepth++;commitPastNavigation()}}
function pastBack(){if(state.screen==='quiz')saveSession();if(pastInternalDepth>0){history.back();return}if(document.referrer&&new URL(document.referrer).origin===location.origin){history.back();return}pastHome()}
function render(){app.innerHTML=state.screen==='home'?home():state.screen==='result'?result():question();const back=document.getElementById('pastBack');if(back)back.hidden=false;if(state.screen==='home')app.querySelectorAll('.round-card').forEach((card,index)=>{const exam=EXAMS[index],buttons=card.querySelectorAll('.mode');if(buttons[0])buttons[0].textContent=pastModeLabel(exam,'exam');if(buttons[1])buttons[1].textContent=pastModeLabel(exam,'study')})}
const restoredPastSession=restoreSession();
render();pastNavigationReady=true;
if(restoredPastSession){
 const restored=pastNavigationState();
 state.screen='home';state.index=0;state.answers=[];state.revealed=false;
 pastInternalDepth=0;render();commitPastNavigation(true);
 restorePastNavigation(restored);
 pastInternalDepth=1;commitPastNavigation();
}else commitPastNavigation(true);
window.addEventListener('popstate',event=>{if(pendingPastHome){pendingPastHome=false;pastInternalDepth=0;location.replace('../index.html');return}if(event.state&&event.state.pastApp){pastInternalDepth=Math.max(0,Number(event.state.depth)||0);restorePastNavigation(event.state.state)}});
