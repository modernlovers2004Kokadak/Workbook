...
const jpNo=['1','2','3','4'];
let state={page:'home',session:[],idx:0,selected:null,answered:false,view:null,filter:'all',themeFilter:null,search:'',mockActive:false,mockRecorded:false,mockResults:[],questionDesign:''};
...
function navigationState(){return{...state,session:state.session.map(q=>q.id),view:state.view?JSON.parse(JSON.stringify(state.view)):null,scrollY:window.scrollY}}
...
function appHome(){if(internalDepth>0){pendingProblemHome=true;history.go(-internalDepth);return}openFirstLevel()}
...
function startSession(arr,page='question',mock=false,questionDesign=''){if(!arr||!arr.length){alert('出題できる問題がありません。');route('home');return}if(navigationReady)commitNavigation(true);state={...state,page,session:arr,idx:0,selected:null,answered:false,view:null,mockActive:mock,mockRecorded:false,mockResults:[],questionDesign};if(mock)saveMockProgress();render();if(navigationReady){internalDepth++;commitNavigation()}}
...
function appBack(){if(state.mockActive&&state.page==='question')saveMockProgress();if(internalDepth>0){history.back();return}if(document.referrer&&new URL(document.referrer).origin===location.origin){history.back();return}openFirstLevel()}
