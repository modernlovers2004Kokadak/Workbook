...
const jpNo=['1','2','3','4'];
let state={page:'home',session:[],idx:0,selected:null,answered:false,view:null,filter:'all',themeFilter:null,search:'',mockActive:false,mockRecorded:false,mockResults:[],questionDesign:'',returnState:null};
const HOME_MODE=new URLSearchParams(location.search).get('mode')||'all';
let navigationReady=false,internalDepth=0,pendingProblemHome=false,initialDirectQuestion=false,initialRestoredMock=false;
function navigationState(){return{...state,session:state.session.map(q=>q.id),view:state.view?JSON.parse(JSON.stringify(state.view)):null,returnState:state.returnState?JSON.parse(JSON.stringify(state.returnState)):null,scrollY:window.scrollY}}
function restoreNavigationState(saved){
 if(!saved)return false;
 state={...state,...saved,session:(saved.session||[]).map(qById).filter(Boolean)};
 render();requestAnimationFrame(()=>window.scrollTo(0,Number(saved.scrollY)||0));return true;
}
function commitNavigation(replace=false){
 const row={problemApp:true,depth:internalDepth,state:navigationState()};
 try{history[replace?'replaceState':'pushState'](row,'',location.href)}catch(_){}
}
function openFirstLevel(){location.replace('../index.html')}
function appHome(){storageRemove('riyo_navigation_state_v1');location.replace('../index.html')}
...
function startCatSession(id){const p=prog(),arr=qsCat(id).filter(q=>!isMastered(p,q));if(arr.length)startSession(arr,'question',false,'unified');else alert('未克服の問題はありません。')}
...
function materialUrl(q){
  ...
  return Number.isInteger(law)?`../学習資料/index.html#law=${law}`:'../学習資料/index.html';
}
function materialCta(q){return `<a class="material-confirm-link" href="${materialUrl(q)}">📖 教材</a>`;}
function initFromHash(){
  if(restoreMockProgress()){initialRestoredMock=true;return;}
  const hash=decodeURIComponent(location.hash||'');
  const direct=hash.match(/question=(\d+)/);
  if(direct){
    const q=qById(direct[1]);
    if(q){initialDirectQuestion=true;startSession([q],'question',false,'unified');return;}
  }
  const m=hash.match(/cat=([^&]+)/);
  if(m){
    const id=m[1];
    const p=prog(),arr=qsCat(id).filter(q=>!isMastered(p,q));
    if(arr.length){startSession(arr,'question',false,'unified');return;}
  }
...
function saveMockProgress(){if(!state.mockActive)return;const nextIndex=state.idx+(state.answered?1:0);store.set('mockProgress',{sessionIds:state.session.map(q=>q.id),idx:nextIndex,results:state.mockResults||[],completed:nextIndex>=state.session.length});}
function clearMockProgress(){storageRemove(PREFIX+'mockProgress')}
function restoreMockProgress(){const saved=store.get('mockProgress');if(!saved||!Array.isArray(saved.sessionIds)||!Array.isArray(saved.results))return false;const session=saved.sessionIds.map(qById).filter(Boolean);if(session.length!==55||saved.results.length>55){clearMockProgress();return false}const idx=Math.min(Number(saved.idx)||0,session.length-1);state={...state,page:saved.completed?'score':'question',session,idx,selected:null,answered:false,view:null,mockActive:true,mockRecorded:false,mockResults:saved.results};saveMockProgress();render();return true}
function startOrResumeMock(){if(state.mockActive&&state.session.length===55){state.idx=Math.min(state.mockResults.length,state.session.length-1);state.page=state.mockResults.length>=55?'score':'question';state.answered=false;state.selected=null;state.view=null;render();internalDepth++;commitNavigation();return}if(restoreMockProgress()){internalDepth++;commitNavigation();return}startSession(mockSession(),'question',true)}
function startSession(arr,page='question',mock=false,questionDesign=''){if(!arr||!arr.length){alert('出題できる問題がありません。');route('home');return}const returnState=state.page==='question'||state.page==='score'?(state.returnState||null):navigationState();if(navigationReady)commitNavigation(true);state={...state,page,session:arr,idx:0,selected:null,answered:false,view:null,mockActive:mock,mockRecorded:false,mockResults:[],questionDesign,returnState};if(mock)saveMockProgress();render();if(navigationReady){internalDepth++;commitNavigation()}}
function route(page,extra={}){if(page===state.page&&!Object.keys(extra).length)return;if(navigationReady)commitNavigation(true);state={...state,page,...extra,selected:null,answered:false,view:null};render();if(navigationReady){internalDepth++;commitNavigation()}}
const circled=['①','②','③','④','⑤','⑥','⑦','⑧','⑨','⑩','⑪','⑫','⑬','⑭','⑮'];
...
function header(title='理容師国家試験 問題集'){const searchable=['home','mistakes','mastered','search'].includes(state.page);return `<header class="top"><span class="top-spacer"></span><h1>${title}</h1>${searchable?`<button onclick="route('search')" aria-label="検索">⌕</button>`:`<span class="top-spacer"></span>`}</header>`}
...
function question(){let q=currentQ(),b=base(q),c=cat(b.gid),s=sub(q.subject),ci=state.view.choices.findIndex(x=>x.correct),ok=state.answered&&state.view.choices[state.selected]?.correct;return `<main class="unified-question">${header(`${state.mockActive?'模試':'第'+(state.idx+1)+'問'} / ${state.session.length}問`,true)}${state.answered?`<div class="resultTop ${ok?'ok':'ng'}">${ok?'⭕':'❌'}<span>${ok?'正解':'不正解'}</span>${!ok?`<span>正解 ${jpNo[ci]} ${state.view.choices[ci].text}</span>`:''}</div>`:''}<section class="content questionContent ${state.answered?'withResult':''}"><div class="question-progress" aria-hidden="true"><i style="width:${(state.idx+1)/state.session.length*100}%"></i></div><div class="qcard question-card ${importanceClass(q)}"><div class="qmeta"><span>${s.name} ＞ ${c.name}</span><span>${importanceBadge(q).replace('・4択','')}</span></div><p class="qtext">${displayQuestionText(b.question)}</p>${state.view.choices.map((ch,i)=>choiceBtn(ch,i)).join('')}</div>${state.answered?answerBox(q,ci,b):''}</section><div class="fixedNext"><button class="${state.answered?'nextBtn':''}" onclick="${state.answered?'nextQ()':'submitAnswer()'}">${state.answered?'次へ':'解答する'}</button></div>${bottom('today')}</main>`}
...
function selectChoice(i){if(!state.answered){state.selected=i;render();commitNavigation(true)}}
function submitAnswer(){if(state.selected===null)return;const q=currentQ(),ok=!!state.view.choices[state.selected]?.correct;record(q,ok);if(state.mockActive)state.mockResults.push({id:q.id,ok});state.answered=true;if(state.mockActive)saveMockProgress();render();commitNavigation(true)}
function nextQ(){if(state.idx<state.session.length-1){state={...state,page:'question',idx:state.idx+1,selected:null,answered:false,view:null};if(state.mockActive)saveMockProgress();render();commitNavigation(true)}else{state={...state,page:'score',selected:null,answered:false,view:null};render();commitNavigation(true)}}

function mastered(){const p=prog(),list=masteredList(p).filter(q=>state.filter==='all'||groupId(q)===state.filter);return `<main>${header('克服済み',true)}<section class="content"><div class="chips"><button onclick="state.filter='all';render();commitNavigation(true)">すべて</button>${CATEGORIES.map(c=>`<button onclick="state.filter='${c.id}';render();commitNavigation(true)">${c.name}</button>`).join('')}</div>${list.length?list.map(q=>masteredRow(q,p)).join(''):'<div class="empty">克服済みの問題はありません。</div>'}</section>${bottom('mastered')}</main>`}
...
function mistakes(){const p=prog(),list=needReviewList(p).filter(m=>state.filter==='all'||groupId(qById(m.id)||{})===state.filter);return `<main>${header('🔖 要復習',true)}<section class="content"><div class="chips"><button onclick="state.filter='all';render();commitNavigation(true)">すべて</button>${CATEGORIES.map(c=>`<button onclick="state.filter='${c.id}';render();commitNavigation(true)">${c.name}</button>`).join('')}</div>${list.length?list.map(m=>mistakeRow(m)).join(''):'<div class="empty">要復習の問題はありません。</div>'}</section>${bottom('mistake')}</main>`}
function mistakeRow(m){const q=qById(m.id),b=base(q);return `<button class="law" onclick="startSession([qById(${m.id})],'question',false,'review')"><span class="num">🔖</span><span class="grow"><span>${cat(b.gid).name}／${b.point}</span><small>登録中　ミス ${m.wrongCount}回　最終 ${m.lastMiss||m.firstMiss||'記録なし'}</small></span><span>解く ›</span></button>`}
...
function appBack(){if(state.mockActive&&state.page==='question')saveMockProgress();if((state.page==='question'||state.page==='score')&&document.referrer.includes('/学習資料/')){location.replace(document.referrer);return}if((state.page==='question'||state.page==='score')&&state.returnState){const saved=state.returnState;state={...state,...saved,session:(saved.session||[]).map(qById).filter(Boolean),returnState:null};render();scrollTo(0,Number(saved.scrollY)||0);commitNavigation(true);return}if(state.page!=='home'){state={...state,page:'home',session:[],idx:0,selected:null,answered:false,view:null,mockActive:false,mockRecorded:false,mockResults:[],returnState:null};render();scrollTo(0,0);commitNavigation(true);return}openFirstLevel()}
function syncProblemUi(){const back=document.getElementById('problemBack'),nav=back?.closest('.floating-nav');if(back)back.hidden=false;if(nav)nav.classList.toggle('with-fixed-next',state.page==='question');if(state.page!=='home')return;const content=app.querySelector('.content');if(content&&!content.querySelector('.problem-description')){const description=document.createElement('p');description.className='problem-description';description.textContent=`${QUESTIONS.length}問の独自問題`;content.prepend(description)}const mock=app.querySelector('.summary-item.sm-blue'),pending=store.get('mockProgress');if(mock){const label=mock.querySelector(':scope > span:nth-of-type(2)'),meta=mock.querySelector('em');if(label)label.textContent=pending&&!pending.completed?'模試を続きから再開':'模試';if(meta)meta.innerHTML=`問題集から構成する55問模試<br>${mockStatsText(prog())}`}}
function render(){app.innerHTML=({home,question,mistakes,mastered,search:searchPage,score,settings}[state.page]||home)();syncProblemUi()}
document.addEventListener('DOMContentLoaded',syncProblemUi);
initFromHash();
navigationReady=true;
if(initialRestoredMock){
 const restored=navigationState();
 state={...state,page:'home',session:[],idx:0,selected:null,answered:false,view:null,mockActive:false,mockRecorded:false,mockResults:[],questionDesign:''};
 internalDepth=0;commitNavigation(true);
 state={...state,...restored,session:(restored.session||[]).map(qById).filter(Boolean)};
 internalDepth=1;render();commitNavigation();
}else if(initialDirectQuestion&&(!document.referrer||new URL(document.referrer).pathname===new URL('../index.html',location.href).pathname)){
 const direct=navigationState();
 state={...state,page:'home',session:[],idx:0,selected:null,answered:false,view:null,mockActive:false,mockRecorded:false,mockResults:[],questionDesign:''};
 internalDepth=0;commitNavigation(true);
 state={...state,...direct,session:(direct.session||[]).map(qById).filter(Boolean)};
 internalDepth=1;render();commitNavigation();
}else commitNavigation(true);
window.addEventListener('popstate',event=>{
 if(pendingProblemHome){pendingProblemHome=false;internalDepth=0;openFirstLevel();return}
 if(event.state&&event.state.problemApp){internalDepth=Math.max(0,Number(event.state.depth)||0);restoreNavigationState(event.state.state)}
});
