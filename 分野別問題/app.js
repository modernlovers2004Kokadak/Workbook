...
const HOME_MODE=new URLSearchParams(location.search).get('mode')==='category'?'category':'category';
...
function header(title='理容師国家試験 法令集'){const searchable=['home','mastered','search'].includes(state.page);return `<header class="top"><span class="top-spacer"></span><h1>${title}</h1>${searchable?`<button onclick="route('search')" aria-label="検索">⌕</button>`:`<span class="top-spacer"></span>`}</header>`}
...
function catRow(c,i,p){const qs=qsCat(c.id);return `<button class="subject-row" type="button" onclick="startCatSession('${c.id}')"><span><span>${circled[i]||String(i+1)} ${esc(c.name)}</span><small>${rankText(qs)}</small>${categoryProgress(c,i,p)}</span><span aria-hidden="true">›</span></button>`}
...
function recordMockResult(pass){
 ...
 function home(){const p=prog();const categoryArea=`<section class="problem-category-list">${CATEGORIES.map((c,i)=>catRow(c,i,p)).join('')}</section>`;return `<main>${header()}<section class="content today-page">${categoryArea}</section>${bottom('home')}</main>`}
