...
function materialHome(){location.replace('../index.html')}
...
function quizLinkFor(li, article){return `../分野別問題/index.html#cat=${encodeURIComponent(quizCategoryForLaw(li,article))}`;}
...
function materialBack(){const detail=document.getElementById('detail');if(detail&&!detail.classList.contains('hidden')){detail.classList.add('hidden');show(detailReturnScreen||'home',false);updateMaterialNav();commitMaterialNavigation(true);return}if(currentScreen!=='home'){show('home',false);commitMaterialNavigation(true);return}materialHome()}
